...
read -p "ARE YOU SURE YOU WISH TO FRESH INSTALL? [Script: WemxPRO-INSTALLER-1.0] (y/N) " yn

case $yn in
	[yY] ) echo Continuing with the NEW INSTALL;
		break;;
	[nN] ) echo Exiting...;
		exit;;
...
warning "IF YOU DID NOT MEAN TO RUN THE INSTALLER"
...
read -sp "Enter the License Key for WEMX: " WEMX_KEY
...
sleep 3s
warning "You may need to enter/paste the key from wemx again..."
...
warning "At this moment, WemX only supports $ USD if you are using a different currency on the previous version, this might cause some issues. More currency support will be added."
...
echo "Choose a command to run:"
echo "============================================================================"
echo "1. sudo apt install -y certbot (NekoHosting's Recommendations)"
echo "2. sudo apt install -y python3-certbot-nginx (Never seems to work but not stopping you)"
...
echo "3. HTTP challenge Nginx"
echo "4. cancel script and exit."
echo "============================================================================"
...
do
    case $opt in
        ...
        "2")
            ...
            break;;
        "3")
            sudo certbot certonly --nginx -d $DOMAIN_URL
            break;;			
        "4")
            break;;
        *) echo "Invalid option $REPLY";;
